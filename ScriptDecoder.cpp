...
ScriptDecoder* ScriptDecoder::chooseDecoderForTextFromList(const char* buffer, ScriptDecoder** decoders, size_t n)
{
    ...
    int i, b = 0;
    for (int k = 0; k < n; k++) {
printf("chooseDecoderForTextFromList, decoders[%d], %s, %d\n", k, decoders[k]->getName(), size);
        i = 0;
        while(i < size) {
printf("chooseDecoderForTextFromList, decoders[%d], %s, i == %d\n", k, decoders[k]->getName(), i);
...
#else
    bool b1 = isChinese(buffer);
    bool b2 = JapaneseDecoder::canConvertNextChar(buffer, outNumBytes);
printf("ChineseDecoder::canConvertNextChar, %d, %d\n", b1?1:0, b2?1:0);
    return b1 || b2;
#endif
}
...
{
//FIXME: if arm gcc, char is unsigned char. but x86_64 gcc not
    unsigned int n = (*buffer & 0xFF) << 8 | (buffer[1] & 0xFF);

printf("ChineseDecoder::isChinese, %X\n", n);
...
printf("ChineseDecoder::isChinese a1a1 == %d, %d\n", 
(buffer[1] >= 0xA1)?1:0, (buffer[1] <= 0xFF)?1:0);
